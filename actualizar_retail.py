...
def fetch_data(client, desde, hasta):
    ...
    query = f"""
        ...
        WHERE Fecha BETWEEN '{desde}' AND '{hasta}'
          AND SAFE_CAST(Facturacion AS FLOAT64) BETWEEN 0 AND 1e12  -- excluye negativos (devoluciones); confirmar si aplica
        ORDER BY Fecha DESC
    """
...
def fetch_objetivos_data(client=None):
    ...
    _io = hl.get('objetivo_ordenes_bq');     idx_ord = _io if _io is not None else next((hl[k] for k in hl if 'ord' in k and 'order' not in k), None)
    ...
    result_dict = {}
    for row in values[1:]:
        marca = cell(row, idx_marca)
        mes   = cell(row, idx_mes)[:7]   # normalizar a YYYY-MM
        fac_s = cell(row, idx_fac).replace(".", "").replace(",", ".")
        ord_s = cell(row, idx_ord).replace(".", "").replace(",", ".")
...
def _objetivos_fallback():
    # No usar datos hardcodeados desactualizados — devolver vacío para que
    # el dashboard muestre "sin objetivos" en lugar de datos incorrectos.
    print("  ERROR: no se pudo mapear columnas de objetivos — dashboard mostrará sin objetivos.")
    return {}
...
def fetch_locales_obj(client):
    ...
    for row in values[1:]:
        ...
        ord_s     = cell(idx_ord_s).replace(".", "").replace(",", ".") if idx_ord_s is not None else ""
        ...
        try:
            ...
            continue
        try:
            obj_ord_local = round(float(ord_s)) if ord_s else 0
        except ValueError:
            obj_ord_local = 0

        key = (b, local_raw.upper().strip())
        obj_dict.setdefault(key, {})[mes] = (obj_M, obj_ord_local)
    ...
    q = f"""
        SELECT
            ...
            COUNT(DISTINCT Orden) AS real_ord
        FROM `{PROJECT_ID}.{DATASET_ID}.{TABLE_VENTAS}`
    ...
    # Index BQ results: (b, local_upper) -> {mes: (real_M, real_ord)}
    real_dict = {}
    for r in bq_rows:
        ...
        real_dict.setdefault(key, {})[r.mes] = (round(float(r.real_M or 0), 1), int(r.real_ord or 0))
    ...
    for (b, local_upper), obj_months in obj_dict.items():
        ...
        d = {}
        for mes, obj_data in obj_months.items():
            obj_M, obj_ord_local = obj_data if isinstance(obj_data, tuple) else (obj_data, 0)
            if obj_M > 0:
                real_data = real_months.get(mes, (0.0, 0))
                real_M, real_ord = real_data if isinstance(real_data, tuple) else (real_data, 0)
                d[mes] = [round(real_M, 1), round(obj_M, 1), int(real_ord), int(obj_ord_local)]
        if d:
...
def compute_top10(base_rows, latest_mes):
    ...
    for pk, months in periods.items():
        ...
        for r in base_rows:
            ...
            agg[key]["ord"] += r["ord"]
            agg[key]["tot"] += r["tot"]
        # Todos los locales por marca, ordenados por facturación
        by_marca = {}
        for (marca, local), v in agg.items():
            by_marca.setdefault(marca, []).append((local, v))
        for marca, entries in by_marca.items():
            for local, v in sorted(entries, key=lambda x: -x[1]["fac"]):
...
def fetch_royalty_data():
    ...
    try:
        ...
        service = build("sheets", "v4", credentials=creds, cache_discovery=False)
        result  = service.spreadsheets().values().get(
            spreadsheetId=SHEET_ID, range=f"{SHEET_NAME}!A1:V30"
        ).execute()
    ...
    # 9=Patagonia %Obj, 10=Patagonia Var, 11=Feriado GMV, 12=Feriado ROY, 13=Feriado %,
    # 14=Feriado %Obj, 15=Feriado Var, 16-20=Total Retail
    COLS = {"Temple":(1,2,3), "Patagonia":(6,7,8), "Feriado":(11,12,13)}
